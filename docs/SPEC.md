...
4. refresh-results: task skill with `disable-model-invocation: true`. Run `make report`, regenerate the README block between `<!-- RESULTS:START -->` and `<!-- RESULTS:END -->` from reports/results.json, then run quant-reviewer on the diff.
